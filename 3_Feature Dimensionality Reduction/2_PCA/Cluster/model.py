...
class model(object):
    ...
    def build(self,model_type):
        ...
        if self.model_type=="kmeans":
            ...
            print("选用算法birch")
            self.estimator=Birch(n_clusters =self.class_num,threshold=0.1)   #  参数设置threshold=0.3,branching_factor=70,leaf_node_entries=70
        else:
    ...
    def show(self,file_list):
        ...
        if glob.glob("result/%s"%self.model_type)==[]:
            ...
            os.makedirs("result/%s"%self.model_type)
        for label in set(self.label_pred):
            if glob.glob("result/%s/%s"%(self.model_type,label))!=[]:
                ...
                shutil.rmtree("result/%s/%s"%(self.model_type,label))
            else:
                os.makedirs("result/%s/%s"%(self.model_type,label))
        for file_path,label in zip(file_list,self.label_pred):
            # print(file_path,label)
            # exit()
            name = file_path.split('/')[-1]
            # print(file_path,'====',"result/%s/%s/%s"%(self.model_type,label,name))
            # exit()
            shutil.copy(file_path,"result/%s/%s/%s"%(self.model_type,label,name))
